...
void removeTail(List *&L)
{
    if (L != nullptr && L->p_head != nullptr)
    {
        NODE* prevTail = L->p_head;
        NODE* xoa = L->p_tail;
        if (L->p_head == L->p_tail) L->p_head = L->p_tail = nullptr;
        else
        {
            while (prevTail->p_next != L->p_tail)
            {
                prevTail = prevTail->p_next;
            }
            L->p_tail = prevTail;
...
void removeBefore(List *&L, int val)
{
    ...
    {
        NODE* xoa = L->p_head;
        while (xoa->p_next != nullptr && xoa->p_next->key != val)
        {
            xoa = xoa->p_next;
        }
        if (xoa != L->p_tail)
        {
            if (xoa == L->p_head) removeHead(L);
            else
            {
                NODE* temphead = L->p_head;
                while (temphead->p_next != xoa)
                {
                    temphead = temphead->p_next;
                }
                temphead->p_next = temphead->p_next->p_next;
                delete xoa;
            }
        }
    }
    ...
    else if (pos > 0 && L != nullptr && L->p_head != nullptr)
    {
        ...
        if (i == pos)
        {
            ...
            else
            {
                ...
                temphead->p_next = createNode(data);
                temphead->p_next->p_next = after;
            }
            return true;
        }
    }
...
bool addBefore(List *&L, int data, int val)
{
    if (L != nullptr && L->p_head != nullptr)
    {
        ...
        {
            NODE* add = L->p_head;
            while (add->p_next != nullptr && add->p_next->key != val)
            {
                add = add->p_next;
            }
            if (add->p_next != nullptr)
            {
                NODE* temp = add->p_next;
                add->p_next = createNode(data);
                add->p_next->p_next = temp;
                return true;
            }
        }
    }
...
void printList(List *L)
{
    NODE* printed = L->p_head;
    while (printed != nullptr)
    {
        cout << printed->key << "->";
        printed = printed->p_next;
    }
    cout << "NULL";
}
