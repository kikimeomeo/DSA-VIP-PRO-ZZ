...
bool addHead(List *&L, int data)
{
    ...
    {
        L->p_head = L->p_tail = createNode(data);
    }
    else
    {
...
void removeTail(List *&L)
{
    ...
    {
        NODE* xoa = L->p_tail;
        if (L->p_head == L->p_tail) L->p_head = L->p_tail = nullptr;
        else
        {
...
void removeAll(List *&L)
{
    if (L != nullptr && L->p_head != nullptr)
    {
        NODE* temphead = L->p_head;
        NODE* xoa;
        while (temphead != nullptr)
        {
            xoa = temphead;
            temphead = temphead->p_next;
            delete xoa;
        }
        L->p_head = L->p_tail = nullptr;
    }
...
void removeBefore(List *&L, int val)
{
    if (L != nullptr)
    {
        NODE* temptail = L->p_tail;
        while (temptail != nullptr && temptail->key != val) 
        {
            temptail = temptail->p_prev;
        }
        if (temptail != nullptr && temptail->p_prev != nullptr)
        {
            NODE* xoa = temptail->p_prev;
            if (xoa == L->p_tail)
            {
                L->p_head = temptail;
                L->p_head->p_prev = nullptr;
            ...
            {
                NODE* beforexoa = xoa->p_prev;
                temptail->p_prev = beforexoa;
                beforexoa->p_next = temptail;
            }
            delete xoa;
        }
    }
...
void removeAfter(List *&L, int val)
{
    ...
    {
        NODE* temphead = L->p_head;
        while (temphead != nullptr && temphead->key != val) 
        {
            temphead = temphead->p_next;
        }
        if (temphead != nullptr && temphead->p_next != nullptr)
        {
            NODE* xoa = temphead->p_next;
            if (xoa == L->p_tail)
            {
                L->p_tail = temphead;
                L->p_tail->p_next = nullptr;
            ...
            {
                NODE* afterxoa = xoa->p_next;
                temphead->p_next = afterxoa;
                afterxoa->p_prev = temphead;
            }
            delete xoa;
        }
    }
...
bool addPos(List *&L, int data, int pos)
{
    ...
    else if (pos > 0 && L != nullptr && L->p_head != nullptr)
    {
        int i = 1;
        NODE* temphead = L->p_head;
        while (i < pos && temphead->p_next != nullptr)
        {
            ++i;
            temphead = temphead->p_next;
        }
        if (i == pos)
        {
            ...
            {
                NODE* after = temphead->p_next;
                NODE* newnode = createNode(data);
                temphead->p_next = newnode;
                newnode->p_prev = temphead;
                newnode->p_next = after;
                after->p_prev = newnode;
            }
            return true;
        }
    }
...
void removePos(List *&L, int data, int pos)
{
    ...
    else if (L != nullptr && L->p_head != nullptr)
    {
        int i = 1;
        NODE* temphead = L->p_head;
        while (i < pos && temphead->p_next != nullptr)
        ...
        if (i == pos && temphead != L->p_tail)
        {
            NODE* xoa = temphead->p_next;
            if (temphead->p_next == L->p_tail)
            {
                L->p_tail = temphead;
                L->p_tail->p_next = nullptr;
            }
            ...
            {
                NODE* after = xoa->p_next;
                temphead->p_next = after;
                after->p_prev = temphead;
            }
            delete xoa;
        }
    }
...
bool addBefore(List *&L, int data, int val)
{
    if (L != nullptr && L->p_head != nullptr)
    {
        ...
        else
        {
            ...
            {
                NODE* newnode = createNode(data);
                NODE* before = add->p_prev;
                add->p_prev = newnode;
                newnode->p_next = add;
                before->p_next = newnode;
                newnode->p_prev = before;
                return true;
            }
        }
    }
...
void printList(List *L)
{
    if (L != nullptr)
    {
        if (L->p_head != nullptr) cout << "NULL" << "<-" << L->p_head;
        else return;
        if (L->p_head->p_next != nullptr)
        {
            NODE* printed = L->p_head->p_next;
...
void removeDuplicate(List *&L)
{
    if (L != nullptr && L->p_head != nullptr && L->p_head->p_next != nullptr)
    {
        ...
        while (temphead != nullptr)
        {
            if (!exist[temphead->key]) 
            {
                exist[temphead->key] = true;
            }
            else
            {
                removeElement(L, temphead->key);
            }
            temphead = temphead->p_next;
        }  
    }
...
bool removeElement(List *&L, int key)
{
    if (L != nullptr && L->p_head != nullptr)
    {
        ...
        {
            while (xoa->p_next != nullptr && xoa->p_next->key != key)
            {
                xoa = xoa->p_next;
            }
            if (xoa->p_next != nullptr)
            {
                if (L->p_tail == xoa->p_next)
                {
                    L->p_tail = xoa;
                    xoa = xoa->p_next;
                    L->p_tail->p_next = nullptr;
